...
class BaseParser(scrapy.Spider):
    ...
    def get_salary(jo):
        ...
        return {'salary': {'min': min_salary, 'max': max_salary, 'per_time': per_time, 'currency': currency}}
...
class HhSpider(BaseParser):
    """
    Нужны мидлвары обработки результата, а также pydantic модель на сервека
